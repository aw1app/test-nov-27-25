...
public class ServletB extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ( session==null ) {
			...
			session  = request.getSession(true); // true create a new session object it it did not exist
		}else {
		...
		// let's track how many this user has visited the web site across all servlets.
		Integer userVisitCounter = (Integer) session.getAttribute("visit-count");
		if(userVisitCounter==null)userVisitCounter=0;
		else
			userVisitCounter++;	
